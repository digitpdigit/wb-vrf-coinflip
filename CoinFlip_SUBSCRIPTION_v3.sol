...
contract CoinFlip is VRFConsumerBaseV2, Ownable {
    ...
    mapping(address => uint256) public addressToTreasury;
    ...
    function checkFlip(uint256 _randomNumber, uint256 _requestId) internal {
        ...
        if (isWin) {
            ...
            // TF REWARD
            addressToReward[c.betStarter] += c.totalGain;
            emit CoinFlipped(
                c.ID,
                c.bet,
                c.reward,
            ...
            // TF REST TO TREASURY
            addressToTreasury[c.betStarter] += c.loseToTreasuryValue;
            emit CoinFlipped(
                c.ID,
                c.bet,
                c.reward,
    ...
    function claimReward() public {
        _tokenContract.transfer(
            ...
            addressToTreasury[msg.sender]
        );
        _tokenContract.transfer(msg.sender, addressToReward[msg.sender]);

        addressToTreasury[msg.sender] = 0;
    ...
    function burnTax() public {
        // CHECK TREASURY BALANCE
        require(
            _tokenContract.balanceOf(address(this)) >= BURNT_AMOUNT,
            "Not enough treasury"
        );
