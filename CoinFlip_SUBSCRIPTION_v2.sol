...
contract CoinFlip is VRFConsumerBaseV2, Ownable {
    ...
    address public TREASURY_ADDRESS = 0x0000000000000000000000000000000000000000;
    uint256 public TOTAL_BURNT_AMOUNT = 0;
    uint256 public BURNT_AMOUNT = 0;
    ...
    uint32 private callbackGasLimit = 50000;
    ...
    struct CoinFlipStruct {
        ...
        uint256 loseBurnTax;
        uint256 winTaxValue;
        uint256 loseBurnTaxValue;
        uint256 loseToTreasuryValue;
    }
    ...
    constructor(uint64 subscriptionId, address tokenAddress, address burnerAddress, address treasuryAddress) VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        s_owner = msg.sender;
        s_subscriptionId = subscriptionId;
        _tokenContract = IERC20(tokenAddress);
        BURNER_ADDRESS=address(burnerAddress);
        TREASURY_ADDRESS=address(treasuryAddress);
    }

  // Choice = 1 | 0 => 1 = Head, 0 = Tail
  function coinFlip(uint256 amountCHRO, uint8 choice, uint32 callbackGas, uint16 requestConfirmation) public {
        address theBetStarter = msg.sender;
        uint256 reward = amountCHRO * (2 * 1000 - winTax) / 1000;
        uint256 coinFlipID = coinFlipIDCounter;

        // CHECK TOKEN BALANCE
        require(_tokenContract.balanceOf(address(theBetStarter)) >= amountCHRO, "Not enough balance");
        require(
            _tokenContract.allowance(address(theBetStarter), address(this)) >= amountCHRO,
        ...
        requestIdToFlipStructs[requestId] = CoinFlipStruct(
          ...
          winTax,
          loseBurnTax,
          amountCHRO * winTax / 1000,
          amountCHRO * loseBurnTax / 1000,
          amountCHRO * (1000 - loseBurnTax) / 1000
        );
    ...
    function checkFlip(uint256 _randomNumber, uint256 _requestId) internal {
      // CHECK FLIP
        ...
        if(isWin){
            ...
            // BURN BY LOSE TAX
             _recordBurnTax(c.loseBurnTaxValue);

            // TF REST TO TREASURY
            _tokenContract.transfer(address(TREASURY_ADDRESS), c.bet - (c.bet * c.loseBurnTax / 1000));
            emit CoinFlipped(c.ID, c.bet, c.reward, c.betStarter, address(this), isWin, c.choice, result, block.timestamp);
        }
    }

    function _recordBurnTax(uint256 amount) internal{
      BURNT_AMOUNT += amount;
    }

    function burnTax() public {
      // CHECK TREASURY BALANCE
      require(_tokenContract.balanceOf(address(this)) >= BURNT_AMOUNT, "Not enough treasury");

      _tokenContract.transfer(address(BURNER_ADDRESS), BURNT_AMOUNT);

      TOTAL_BURNT_AMOUNT += BURNT_AMOUNT;
      BURNT_AMOUNT = 0;
    }
